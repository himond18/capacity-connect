/* Capacity Connect — server is source of truth, without destructive empty-syncs */
(function(){
 ...
 function normalizeCourse(c){
  if(!c)return null;
  return {
   id:c.id,title:c.title||'Course',subject:c.subject||'',description:c.description||'',
   trainer:c.trainer||c.trainer_name||'',
   published:String(c.status||'').toLowerCase()==='published'||c.published===true,
   status:String(c.status||'draft').toLowerCase(),
   enrollmentCount:Number(c.enrollment_count||c.enrollmentCount||0),
   recordedClasses:Array.isArray(c.recordedClasses)?c.recordedClasses:[],
   notes:Array.isArray(c.notes)?c.notes:[],
   liveClasses:Array.isArray(c.liveClasses)?c.liveClasses:[]
  };
 }
 function mergeCourses(existing,server){
  const map=new Map();
  (Array.isArray(existing)?existing:[]).map(normalizeCourse).filter(Boolean).forEach(c=>map.set(String(c.id),c));
  (Array.isArray(server)?server:[]).map(normalizeCourse).filter(Boolean).forEach(c=>map.set(String(c.id),c));
 ...
 async function boot(){
  ...
  try{
   ...
   if(typeof current!=='undefined'&&current){
    const page=current.page||'dashboard',courseId=current.courseId;
    current.name=serverUser.name||storedUser.name||'';
    current.role=String(serverUser.role||storedUser.role||'trainee').toLowerCase();
    current.page=page;if(courseId!=null)current.courseId=courseId;
   }
   ...
   if(role==='trainee'){
    ...
    u.enrolled=(enrollments.enrollments||[]).map(e=>e.course_id??e.courseId).filter(x=>x!=null);u.certifications=certs.certifications||[];db.assessments=(assessments.assessments||[]).map(a=>({...a,questions:Array.isArray(a.questions)?a.questions:[]}));db.notifications=notifications.notifications||db.notifications||[];db.courseProgress=progress.progress||db.courseProgress||[];
   }else if(role==='trainer'){
    const previousCourses=Array.isArray(db.courses)?db.courses.slice():[];
